...
const ProductDetails = () => {
    ...
    const fetchProduct = async () => {
      try {
        ...
        if (fetchedProduct.category && fetchedProduct.category.length > 0) {
          const categoryProducts = await productService.getProductsByCategory(fetchedProduct.category[0]);
          setRelatedProducts(categoryProducts.filter(p => p.id !== fetchedProduct.id).slice(0, 4));
        }
      } catch (err) {
  ...
  // Related products are now fetched in the useEffect
